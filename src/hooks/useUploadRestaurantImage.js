...
const useUploadRestaurantImage = ({data, restaurantImage}) => {
  ...
  const upload = async (image) => {
    ...
    try {
      ...
      const storageRef = ref(
        storage,
        `Restaurants/${data.id}/${uniqueId + fileExtension(restaurantImage)}`
      );
      ...
      await addDoc(collectionRef, {
        ...
        type: image.type,
        size: image.size,
        path: `images/${data.id}/${uniqueId + fileExtension(image)}`,
...
export default useUploadRestaurantImage;
